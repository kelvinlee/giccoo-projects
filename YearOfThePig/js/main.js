...
n||(n=1);var i=e.geometry.attributes.position.array,o=[],a;for(a=0;a<e.geometry.attributes.position.array.length;a+=3)o.push(new CANNON.Vec3(i[a]*n,i[a+1]*n,i[a+2]*n));//var _meshShape = new CANNON.ConvexPolyhedron(verts);
var r=new THREE.Geometry;r.fromBufferGeometry(e.geometry);var s=new CANNON.ConvexPolyhedron(o,r.face);t.addShape(s)}function vec3toVector3(e){var t;return new THREE.Vector3(e.x,e.y,e.z);// @codekit-append "../js/phy.js"
...
//=====金条
gift2=objs.gift2.clone();var n=objs.gift2.material.map,i=new THREE.MeshToonMaterial({color:15910477,emissive:15910477,emissiveIntensity:.1});//console.log(pig.material.map)
//var pigMat=new THREE.MeshToonMaterial({map:pigmap,envMap:environment,reflectivity:0.2})
//var pigMat=new THREE.MeshToonMaterial({map:pigmap,shininess:10,specular:0xff3388})//normalMap:pigmap,normalMapType:THREE.ObjectSpaceNormalMap
//var pigMat=new THREE.MeshStandardMaterial({map:pigmap,emissive:0xffffff,envMap:environment,emissiveIntensity:0.5,roughness:0.4,metalness:.5})
gift2.material=i,//TweenMax.to(gift2.material,.5,{emissiveIntensity:0.05,repeat:1000000,yoyo:true})
//=====元宝
gift3=objs.gift3.clone();var o=objs.gift3.material.map,a=new THREE.MeshToonMaterial({color:15910477,emissive:15910477,emissiveIntensity:.1});//console.log(pig.material.map)
//var pigMat=new THREE.MeshToonMaterial({map:pigmap,envMap:environment,reflectivity:0.2})
//var pigMat=new THREE.MeshToonMaterial({map:pigmap,shininess:10,specular:0xff3388})//normalMap:pigmap,normalMapType:THREE.ObjectSpaceNormalMap
//var pigMat=new THREE.MeshStandardMaterial({map:pigmap,emissive:0xffffff,envMap:environment,emissiveIntensity:0.5,roughness:0.4,metalness:.5})
...
scene.add(n),meshes.push(n),TweenMax.from(n.scale,2,{x:.01,y:.01,z:.01,ease:Elastic.easeOut}),e){case 0:var i=new CANNON.Cylinder(3*giftScale,3*giftScale,.5*giftScale,12);//硬币
break;case 1:var i=new CANNON.Sphere(2.5*giftScale);//元宝
break;case 2:var i=new CANNON.Box(new CANNON.Vec3(4.5*giftScale,1.3*giftScale,1.5*giftScale));//金条
break;case 3:var i=new CANNON.Box(new CANNON.Vec3(3*giftScale,4.5*giftScale,.5*giftScale));//红包
break}var o=new CANNON.Body({mass:10,position:new CANNON.Vec3(0,0,0)});//type:CANNON.Body.KINEMATIC
o.addShape(i),n.userData.body=o,n.userData.isGift=!0,o.position.set(pigBody.position.x,pigBody.position.y-20,pigBody.position.z),o.angularVelocity=new CANNON.Vec3(4*Math.random()-2,4*Math.random()-2,4*Math.random()-2),//随机旋转
o.velocity=new CANNON.Vec3(4*Math.random()-2,4*Math.random()-2,4*Math.random()-2),//随机速度
world.add(o),bodies.push(o),SOUND.gift.play(),main.shaked=!0}function aniDone(){console.log("完成")}function setLeftFoot(){leftFoot=objs.foot.clone(),scene.add(leftFoot),leftFoot.scale.set(pigScale,pigScale,pigScale),leftFoot.position.set(0,0,0),leftFoot.castShadow=!0;var e=objs.foot.material.map,t=new THREE.MeshBasicMaterial({map:e});leftFoot.material=t;var n=new THREE.BoxGeometry(1,1,1),i=new THREE.MeshBasicMaterial({color:16776960});leftFootJoint=new THREE.Mesh(n,i),//scene.add(leftFootJoint)
...
function setRightHand(){rightHand=objs.foot.clone(),scene.add(rightHand),rightHand.scale.set(pigScale,pigScale,pigScale),rightHand.position.set(0,0,0),rightHand.castShadow=!0;var e=rightHand.material.map,t=new THREE.MeshBasicMaterial({map:e});rightHand.material=t;var n=new THREE.BoxGeometry(1,1,1),i=new THREE.MeshBasicMaterial({color:16776960});rightHandJoint=new THREE.Mesh(n,i),//scene.add(rightHandJoint)
...
function setLeftHand(){leftHand=objs.foot.clone(),scene.add(leftHand),leftHand.scale.set(pigScale,pigScale,pigScale),leftHand.position.set(0,0,0),leftHand.castShandow=!0;var e=leftHand.material.map,t=new THREE.MeshBasicMaterial({map:e});leftHand.material=t;var n=new THREE.BoxGeometry(1,1,1),i=new THREE.MeshBasicMaterial({color:16776960});leftHandJoint=new THREE.Mesh(n,i),//scene.add(leftHandJoint)
...
function animate(){
//stats.begin()
ifThreePlay&&(requestAnimationFrame(animate),updatePhysics(),render());//stats.end()
}function updatePhysics(){world.step(.05);for(var e=0;e!==meshes.length;e++)meshes[e].position.copy(bodies[e].position),//========meshes[]里放THREE模型，bodies[]里放物理CANNON的 body模型
...
if(e.preventDefault(),mouse.x=e.touches[0].clientX/window.innerWidth*2-1,mouse.y=-e.touches[0].clientY/window.innerHeight*2+1,mouseConstraint){var t=e.touches[0].clientX-lastX,n=e.touches[0].clientY-lastY,i=t<0?-1:1,o=n<0?-1:1;//console.log(pickingPlane)
if(Math.abs(t)>=MIN_DISTANCE_PER_SHAKE&&0!==i&&lastDirX!==i&&(lastDirX=i,entropy+=ENTROPY_PER_SHAKE),Math.abs(n)>=MIN_DISTANCE_PER_SHAKE&&0!==o&&lastDirY!==o&&(lastDirY=o,entropy+=ENTROPY_PER_SHAKE),lastX=e.touches[0].clientX,lastY=e.touches[0].clientY,// if (this.entropy > HIDE_HINT_ENTROPY) {
//     this.hideHint();
// }
...
function loadingMods(e,n,i){var t;modNum++,(new THREE.GLTFLoader).load(e,function(e){
// called when the resource is loaded
modLoadedNum++,console.log(e.scene);// objs[_target]=gltf.scene.children[0]
// if(_ifAddScene){
// 	scene.add(objs[_target])
// }
for(var t=0;t<e.scene.children.length;t++)objs[n[t]]=e.scene.children[t],i&&scene.add(objs[n[t]]),console.log(n[t]);loadingCheck()},// called while loading is progressing
...
var i=new THREE.DirectionalLight(16777215,.3);i.position.set(0,30,0),//dLight.target=scene
i.castShadow=!0,i.shadow.mapSize.width=2048,// default
i.shadow.mapSize.height=2048,// default
i.shadow.camera.near=.5,// default
i.shadow.camera.far=1e3,// default
i.shadow.camera.left=-100,// default
i.shadow.camera.right=100,i.shadow.camera.bottom=-100,// default
i.shadow.camera.top=100,// TweenMax.to(dLight.position,2,{x:100,repeat:100000,yoyo:true,ease:Sine.easeInOut,delay:1})//光源旋转
// TweenMax.to(dLight.position,2,{z:100,repeat:100000,yoyo:true,ease:Sine.easeInOut})
scene.add(i);//====平面
var o=new THREE.PlaneGeometry(600,600,60),a=new THREE.MeshLambertMaterial({color:16774608}),r=new THREE.Mesh(o,a);r.rotation.x=-Math.PI/2,r.position.y=-20,r.receiveShadow=!0,scene.add(r),//====摄像机
...
groundBody=new CANNON.Body({mass:0,shape:groundShape})).quaternion.setFromAxisAngle(new CANNON.Vec3(1,0,0),-Math.PI/2),groundBody.position.set(0,-20,0),world.add(groundBody)}function pixiStart(){setBG(),setPig(),setScorll(),setQR(),setTimeout(openScorll,2e3),setTimeout(showTitle,10),TweenMax.from(pigC,1.6,{y:stageH/5,ease:Bounce.easeOut,delay:1.5}),TweenMax.from(pigC,.5,{alpha:0,delay:1.5})}function setBG(){pStage.addChild(bgC),endbg=new Spr(_CDN+"img/endbg.png"),bgC.addChild(endbg,bgTC),bgTMask=new Spr(_CDN+"img/endtitle.png"),TweenMax.from(endbg,2,{alpha:0}),bgTC.addChild(inkC,bgTMask),bgTC.pivot.set(375,166),bgTC.position.set(375,stageH/2-343),ink1=new Spr(_CDN+"img/ink1.png"),ink2=new Spr(_CDN+"img/ink2.png"),ink3=new Spr(_CDN+"img/ink1.png"),ink4=new Spr(_CDN+"img/ink2.png"),ink5=new Spr(_CDN+"img/ink1.png"),ink6=new Spr(_CDN+"img/ink2.png"),inkC.addChild(ink1,ink2,ink3,ink4,ink5,ink6),ink1.pivot.x=ink3.pivot.x=ink5.pivot.x=324.5,ink1.pivot.y=ink3.pivot.y=ink5.pivot.y=376.5,ink2.pivot.x=ink4.pivot.x=ink6.pivot.x=200,ink2.pivot.y=ink4.pivot.y=ink6.pivot.y=205.5,ink1.scale.x=ink1.scale.y=ink2.scale.x=ink2.scale.y=ink3.scale.x=ink3.scale.y=ink4.scale.x=ink4.scale.y=ink5.scale.x=ink5.scale.y=ink6.scale.x=ink6.scale.y=0,ink1.position.set(-40,-139),ink2.position.set(260,-109),ink3.position.set(388,500),ink4.position.set(408,510),ink5.position.set(836,-212),ink6.position.set(536,-180),//ink1.blendMode=ink2.blendMode=ink3.blendMode=ink4.blendMode=ink5.blendMode=ink6.blendMode=PIXI.BLEND_MODES.MULTIPLY
inkC.mask=bgTMask}function showTitle(){for(var e=[ink4,ink3,ink2,ink1,ink6,ink5],t=0;t<6;t++)TweenMax.from(e[t],1.5,{alpha:0,delay:.05*t}),TweenMax.to(e[t].scale,2,{x:1.9,y:1.9,delay:.05*t})}//======猪
function setPig(){pStage.addChild(pigC),pigC.position.set(375,stageH/2+45),endPig=new Spr(_CDN+"img/endpig.png"),endEyebow=new Spr(_CDN+"img/endeyebow.png"),endPig.pivot.set(375,270.5),endEyebow.pivot.set(375,270.5),pigC.addChild(endPig,endEyebow)}function moveEndPig(){TweenMax.set(endPig,{rotation:-Math.PI/180*12,x:-50}),TweenMax.to(endPig,1,{rotation:Math.PI/180*12,x:50,repeat:1e5,yoyo:!0,ease:Sine.easeInOut}),TweenMax.set(endPig,{y:-20}),TweenMax.to(endPig,.5,{y:10,repeat:1e5,yoyo:!0,ease:Sine.easeInOut}),TweenMax.set(endEyebow,{rotation:-Math.PI/180*12,x:-50}),TweenMax.to(endEyebow,1,{rotation:Math.PI/180*12,x:50,repeat:1e5,yoyo:!0,ease:Sine.easeInOut,delay:.02}),TweenMax.set(endEyebow,{y:-25}),TweenMax.to(endEyebow,.5,{y:15,repeat:1e5,yoyo:!0,ease:Sine.easeInOut,delay:.02}),TweenMax.set(scorllC,{rotation:-Math.PI/180*2,x:-20}),TweenMax.to(scorllC,1,{rotation:Math.PI/180*2,x:20,repeat:1e5,yoyo:!0,ease:Sine.easeInOut,delay:.1}),TweenMax.set(scorllC,{y:16}),TweenMax.to(scorllC,.5,{y:56,repeat:1e5,yoyo:!0,ease:Sine.easeInOut,delay:.1})}//======卷轴
function setScorll(){pigC.addChild(scorllC),scorllC.position.set(0,55),scorllMain=new PIXI.Container,scorllBG=new Spr(_CDN+"img/scorll.png"),scorllT=new Spr(_CDN+"img/scorllt"+parseInt(8*Math.random()+1)+".png"),scorllMain.addChild(scorllBG,scorllT),scorllMain.pivot.set(235,89),(scorllMask=new PIXI.Graphics).beginFill(16777215),scorllMask.drawRect(0,0,470,178),scorllMask.pivot.set(235,89),scorllL=new Spr(_CDN+"img/scorll_l.png"),scorllR=new Spr(_CDN+"img/scorll_l.png"),scorllL.pivot.set(70,122),scorllR.pivot.set(70,122),scorllR.scale.x=-1,scorllC.addChild(scorllMain,scorllMask,scorllL,scorllR),(scorllMain.mask=scorllMask).scale.x=0,moveEndPig()}function openScorll(){TweenMax.to(scorllMask.scale,1.2,{x:1,ease:Back.easeOut}),TweenMax.to(scorllL,1.2,{x:-215,ease:Back.easeOut}),TweenMax.to(scorllR,1.2,{x:215,ease:Back.easeOut}),main.gameEndBtns=!0}//=====完成
function setQR(){QR=new Spr(_CDN+"img/qr.png"),pStage.addChild(QR),QR.y=stageH-199,QR.visible=!1}function showQR(){QR.visible=!0}function shareDone(){QR.visible=!1}var ANIMATION_END_NAME,ANIMATION_END_NAMES,AnimatedSprite,Container,Graphics,IsPC,NumberToChinese,ParticleContainer,SOUND,SectionToChinese,Spr,Sprite,TRANSITION_END_NAME,TRANSITION_END_NAMES,Text,Texture,TextureCache,TrueH,TrueW,UGC,VENDORS,_CDN,_cache,_public,_runTime,_second,_startCache,_testTime,apiLink,apiUrl,autoDetectRenderer,chnNumChar,chnUnitChar,chnUnitSection,css3Prefix,getId,getTe,i,imageurl,init,isAndroid,j,len1,loadWechatConfig,loader,loading,mTestElement,main,musicIconCache,musicLineCache,neteaseShareImage,random,resource,resources,sended,sys,thudIndex,tryThis,ugc,ugcCache;for(VENDORS=["Moz","webkit","ms","O"],TRANSITION_END_NAMES={Moz:"transitionend",webkit:"webkitTransitionEnd",ms:"MSTransitionEnd",O:"oTransitionEnd"},ANIMATION_END_NAMES={Moz:"animationend",webkit:"webkitAnimationEnd",ms:"MSAnimationEnd",O:"oAnimationEnd"},mTestElement=document.createElement("div"),j=0,len1=VENDORS.length;j<len1&&!((css3Prefix=i=VENDORS[j])+"Transition"in mTestElement.style);j++)css3Prefix=!1;css3Prefix&&(TRANSITION_END_NAME=TRANSITION_END_NAMES[css3Prefix],ANIMATION_END_NAME=ANIMATION_END_NAMES[css3Prefix]),loadWechatConfig=function e(){var t,n,i;i=encodeURIComponent(window.location.href.split("#")[0]),(t=document.createElement("script")).src="//api.giccoo.com/api/config?url="+i,(n=document.getElementsByTagName("script")[0]).parentNode.insertBefore(t,n)},IsPC=function e(){var t,n,i,o;for(i=navigator.userAgent,t=new Array("Android","iPhone","SymbianOS","Windows Phone","iPad","iPod"),n=!0,o=0;o<t.length;){if(0<i.indexOf(t[o])){n=!1;break}o++}return n},isAndroid=-1<navigator.userAgent.indexOf("Android")||-1<navigator.userAgent.indexOf("Adr"),//android终端
...
_CDN=".",thudIndex=0,(SOUND={}).runRandomHit=function(){return!!main.canPlay&&(SOUND["hit".concat(thudIndex+1)].play(),thudIndex=(thudIndex+1)%4)},SOUND.hit1=new Howl({src:["".concat(_CDN,"/mp3/pig-1.mp3")]}),SOUND.hit2=new Howl({src:["".concat(_CDN,"/mp3/pig-2.mp3")]}),SOUND.hit3=new Howl({src:["".concat(_CDN,"/mp3/pig-3.mp3")]}),SOUND.hit4=new Howl({src:["".concat(_CDN,"/mp3/pig-4.mp3")]}),SOUND.g=new Howl({src:["".concat(_CDN,"/mp3/gift.mp3")]}),SOUND.giftEnd=new Howl({src:["".concat(_CDN,"/mp3/han.mp3")]}),// volume: 0.5
SOUND.gift={play:function e(){return!!main.canPlay&&SOUND.g.play()}},SOUND.end=function(){return!!main.canPlay&&SOUND.giftEnd.play()},// setTimeout ->
// 	SOUND.bgm.play()
// ,500
loadWechatConfig=function e(){var t,n,i;i=encodeURIComponent(window.location.href.split("#")[0]),(t=document.createElement("script")).src="//api.giccoo.com/api/config?url="+i,(n=document.getElementsByTagName("script")[0]).parentNode.insertBefore(t,n)},IsPC=function e(){var t,n,i,o;for(i=navigator.userAgent,t=new Array("Android","iPhone","SymbianOS","Windows Phone","iPad","iPod"),n=!0,o=0;o<t.length;){if(0<i.indexOf(t[o])){n=!1;break}o++}return n},Vue.component("player",{template:'<div class="player" :class="{play: playing, pause: !playing}" @click="change"> <div class="icon-play" :class="{play: playing, pause: !playing}"> <svg v-if="!icon" v-show="!playing" height="100%" version="1.1" viewBox="0 0 36 36" width="100%"><use class="ytp-svg-shadow" xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#ytp-id-39"></use><path :fill="fill" class="ytp-svg-fill" d="M 12,26 18.5,22 18.5,14 12,10 z M 18.5,22 25,18 25,18 18.5,14 z"></path></svg> <svg v-if="!icon" v-show="playing" height="100%" version="1.1" viewBox="0 0 36 36" width="100%"><use class="ytp-svg-shadow" xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#ytp-id-40"></use><path :fill="fill" class="ytp-svg-fill" d="M 12,26 16,26 16,10 12,10 z M 21,26 25,26 25,10 21,10 z"></path></svg> </div> <audio :src="src" :autoplay="autoplay" :preload="preload" :loop="loop"></audio> <img v-if="thumb" :src="thumb" /> <slot></slot> </div>',data:function e(){return{playing:!1,stoped:!1}},props:{fill:{default:"black"},name:{default:!1},src:{default:"./mp3/bgm.mp3"},thumb:{default:!1},autoplay:{default:!1},preload:{default:!1},loop:{default:!1},callback:{default:!1},icon:{default:!1}},methods:{play:function e(){var t;// @audio.load()
return t=this.$emit("play",this),this.playing=!0,this.audio.play()},pause:function e(){var t;return t=this.$emit("pause",this),this.audio.pause(),this.playing=!1},ended:function e(){return this.playing=!1},change:function e(){return this.playing?(this.audio.pause(),this.stoped=!0):(this.audio.play(),this.stoped=!1)}},
// computed:
mounted:function e(t){
// console.log @name
...
null==Number.isInteger&&(Number.isInteger=function(e){return 0<=e}),Container=PIXI.Container,ParticleContainer=PIXI.ParticleContainer,autoDetectRenderer=PIXI.autoDetectRenderer,loader=PIXI.loader,resources=PIXI.loader.resources,TextureCache=PIXI.utils.TextureCache,Texture=PIXI.Texture,Sprite=PIXI.Sprite,Graphics=PIXI.Graphics,resource=PIXI.loader.resources,Text=PIXI.Text,AnimatedSprite=PIXI.extras.AnimatedSprite,Spr=function e(t){return new Sprite(getTe(t))},getTe=function e(t){return resource[t].texture},getId=function e(t,n){return loader.resources[n].textures[t]},// for fix ios 8 less
null==Number.isInteger&&(Number.isInteger=function(e){return 0<=e}),IsPC=function e(){var t,n,i,o;for(i=navigator.userAgent,t=new Array("Android","iPhone","SymbianOS","Windows Phone","iPad","iPod"),n=!0,o=0;o<t.length;){if(0<i.indexOf(t[o])){n=!1;break}o++}return n},IsPC=function e(){var t,n,i,o;for(i=navigator.userAgent,t=new Array("Android","iPhone","SymbianOS","Windows Phone","iPad","iPod"),n=!0,o=0;o<t.length;){if(0<i.indexOf(t[o])){n=!1;break}o++}return n},// @codekit-prepend "coffee/css3Prefix"
...
main={},sys="",sended=[!(loading={}),!(_public={})],_runTime=_startCache=_cache=ugcCache=musicIconCache=musicLineCache=ugc=null,_testTime=_second=0,neteaseShareImage=function e(){var t,n,i;return i="画山成岳",t="https://image.giccoo.com/upload/".concat(main.folder,"/")+main.shareImageLink+"@!large",n="https://activity.music.163.com/YearOfThePig/",// console.log picUrl,"orpheus://sharepic?picUrl="+encodeURIComponent(picUrl)+"&shareUrl="+encodeURIComponent(redirectUrl)+"&wbDesc="+encodeURIComponent(title1)+"&qqDesc="+encodeURIComponent(title1)
// window.location.href = "orpheus://sharepic?picUrl="+encodeURIComponent(picUrl)+"&shareUrl="+encodeURIComponent(redirectUrl)+"&wbDesc="+encodeURIComponent(title1)+"&qqDesc="+encodeURIComponent(title1)
console.log("share href:",t),CloudMusic.sharePic({picUrl:t,text:i,link:n})},window.onload=function(){var e,t;return TrueH=document.documentElement.clientHeight,TrueW=document.documentElement.clientWidth,e=0,-1<window.navigator.userAgent.indexOf("NeteaseMusic")?(sys="NeteaseMusic",t={name:"YearOfThePig",title:"新年福袋被偷走了，你的福贴还在里面呢! 快点抢回来吧~",subTitle:"可恶的小猪你别跑,把我的福袋还回来!!!",text:"",picUrl:"http://m.giccoo.com/YearOfThePig/img/ico.jpg",link:"http://m.giccoo.com/YearOfThePig/"},CloudMusic.setShareData(t)):(loadWechatConfig(),wx.ready(function(){var e;return e={title:"新年福袋被偷走了，你的福贴还在里面呢! 快点抢回来吧~",desc:"可恶的小猪你别跑,把我的福袋还回来!!!",link:"http://m.giccoo.com/YearOfThePig/",imgUrl:"http://m.giccoo.com/YearOfThePig/img/ico.jpg",success:function e(){},
// alert "success"
...
// clearTimeout _cache
return this.registerShow=!0,// _cache = setTimeout =>
document.getElementById("note-text").innerHTML="<div data-splitting>“对，点这里！”</div><div data-splitting>“咦，咱们俩的手好像啊~”</div>",Splitting()},
// ,400
mute:function e(){if(this.canPlay=!this.canPlay,!this.canPlay)return SOUND.giftEnd.pause(),SOUND.g.pause(),SOUND.hit1.pause(),SOUND.hit2.pause(),SOUND.hit3.pause(),SOUND.hit4.pause()}},mounted:function e(){var t,n;return TrueH=document.documentElement.clientHeight,TrueW=document.documentElement.clientWidth,"NeteaseMusic"===sys&&(this.wy=!0),t=2*TrueH*(2-2*TrueW/750+.01),this.wy=CloudMusic.isInApp(),n=CloudMusic.getClientVersion().split("."),ugc=new UGC({el:"ugc",w:750,h:750/TrueW*TrueH,callback:function e(){}}),document.getElementById("sence").appendChild(initAll())}})},chnNumChar=["零","一","二","三","四","五","六","七","八","九"],chnUnitSection=["","万","亿","万亿","亿亿"],chnUnitChar=["","十","百","千"],SectionToChinese=function e(t){var n,i,o,a,r,s;for(n=o="",i=t,s=!(a=0);0<t;)0===(r=t%10)?s||(s=!0,n=chnNumChar[r]+n):(s=!1,o=chnNumChar[r],n=(o+=chnUnitChar[a])+n),a++,t=Math.floor(t/10);return 10<=i&&i<20?n.substr(1):n},NumberToChinese=function e(t){var n,i,o,a,r;if(n=a="",i=!1,(r=0)===t)return chnNumChar[0];for(;0<t;)o=t%1e4,i&&(n=chnNumChar[0]+n),a=SectionToChinese(o),n=(a+=0!==o?chnUnitSection[r]:chnUnitSection[0])+n,i=o<1e3&&0<o,t=Math.floor(t/1e4),r++;return n},tryThis=function e(t){return console.log("msg:",t)},Fog.prototype.isFog=!0,Fog.prototype.clone=function(){return new Fog(this.color,this.near,this.far)},Fog.prototype.toJSON=function(){return{type:"Fog",color:this.color.getHex(),near:this.near,far:this.far}};var gifts=[],giftsNum=[1,3,5,7,9,16,20,50],gift1,gift2,gift3,gift4,nowGiftType=parseInt(4*Math.random()),giftScale=1,pigScale=.1,footLinearDamping=.2,handLinearDamping=.2,footAngularDamping=.2,handAngularDamping=.2,leftFoot,leftFootBody,leftFootJointBody,leftLegGeo,leftLegMat,leftLeg,rightFoot,rightFootBody,rightFootJointBody,rightLegGeo,rightLegMat,rightLeg,rightHand,rightHandBody,rightHandJointBody,rightArmGeo,rightArmMat,rightArm,leftHand,leftHandBody,leftHandJointBody,leftArmGeo,leftArmMat,leftArm,hanger,headLine,headLineGeo,headLineMat,line=new Float32Array,lines=[],lineMesh,scene=new THREE.Scene,camera=new THREE.OrthographicCamera(window.innerWidth/window.innerHeight*80/-2,window.innerWidth/window.innerHeight*80/2,40,-40,.1,200),renderer=new THREE.WebGLRenderer({antialias:!0,alpha:!1,preserveDrawingBuffer:!0}),modNum=0,modLoadedNum=0,objs={},stats=null,world=new CANNON.World,rootPoint,rootPointBody,pigShape,pigBody,ifFirsTime=!0,cannonDebugRenderer=new THREE.CannonDebugRenderer(scene,world),meshes=[],bodies=[],ifThreePlay=!0,controls,guiControls,datGUI,raycaster=new THREE.Raycaster,mouse=new THREE.Vector2,intersects=[],mouseConstraint=void 0,lastX,lastY,lastDirX,lastDirY,entropy=0,MIN_DISTANCE_PER_SHAKE=20,ENTROPY_PER_SHAKE=1,MAX_ENTROPY=[1,2,3,4,6,8,16,16],level=0,ifFirsTime2=!0,eyeGroup=new THREE.Group,eye1,eye2,eye3,eye4,pickingPlane,spring,isRunningSpring=1,jointBody,groundShape,groundBody,leftFootJoint,leftFootShap,leftFootCurve,rightFootJoint,rightFootShap,rightFootCurve,rightHandJoint,rightHandShap,rightHandCurve,leftHandJoint,leftHandShap,leftHandCurve,_CDN,imageList=[(_CDN="./")+"img/endbg.png",_CDN+"img/endpig.png",_CDN+"img/endeyebow.png",_CDN+"img/endtitle.png",_CDN+"img/scorll_l.png",_CDN+"img/scorll.png",_CDN+"img/scorllt1.png",_CDN+"img/scorllt2.png",_CDN+"img/scorllt3.png",_CDN+"img/scorllt4.png",_CDN+"img/scorllt5.png",_CDN+"img/scorllt6.png",_CDN+"img/scorllt7.png",_CDN+"img/scorllt8.png",_CDN+"img/ink1.png",_CDN+"img/ink2.png",_CDN+"img/qr.png"],pStage,stageH,buildUGC=function e(){console.log("app",this,this.opts.w,this.opts.h),//this.stage.addChild(new Spr(_CDN+"img/endbg.png"))
pStage=this.stage,stageH=this.opts.h},bgC=new PIXI.Container,bgTC=new PIXI.Container,inkC=new PIXI.Container,bgTMask,ink1,ink2,ink3,ink4,ink5,ink6,endbg,pigC=new PIXI.Container,endPig,endEyebow,scorllC=new PIXI.Container,scorllL,scorllR,scorllBG,scorllT,scorllMain,scorllMask,QR;
