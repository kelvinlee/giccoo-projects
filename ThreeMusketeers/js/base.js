...
Note = (function() {
  function Note(type1, name1, stage1, game1, x1, y1, size) {
    var tis;
    this.type = type1;
    this.name = name1;
    this.stage = stage1;
    this.game = game1;
    this.x = x1 != null ? x1 : 150;
    this.y = y1 != null ? y1 : 150;
...
Bullet = (function() {
  function Bullet(name1, Direction1, nums1, x1, y1, speed, w1, h1) {
    var rotation;
    this.name = name1;
    this.Direction = Direction1;
    this.nums = nums1 != null ? nums1 : 0;
    this.x = x1 != null ? x1 : 0;
    this.y = y1 != null ? y1 : 0;
    this.speed = speed != null ? speed : 450;
    this.w = w1 != null ? w1 : 30;
    this.h = h1 != null ? h1 : 2;
    rotation = [2, 0, -2, 0];
    this.body = new createjs.Shape();
    this.body.fromid = this.name;
...
Player = (function() {
  function Player(name1, game1, stage1, x1, y1, width, height, team) {
    this.name = name1;
    this.game = game1;
    this.stage = stage1;
    this.x = x1 != null ? x1 : 150;
  ...
  Player.prototype.my = function() {
    ...
    this.usernameLabel = new createjs.Text("Me", "12px Arial", "#ffffff");
  ...
  Player.prototype.attack = function() {
    ...
    B = new Bullet(this.name, this.Direction, this.mybullets, x, y);
  ...
  Player.prototype.bullet = function() {
    var Curve, blt, bulletlife, card, j, k, len, len1, ref, removeBullet, results;
    removeBullet = [];
    bulletlife = 800;
    Curve = 1.5;
    ref = this.game.bullteGround.children;
    ...
    results = [];
    for (k = 0, len1 = removeBullet.length; k < len1; k++) {
      blt = removeBullet[k];
      results.push(this.game.bullteGround.removeChild(blt));
    }
    return results;
  };

  Player.prototype.Collision = function(blt) {
    var e, enemy, enemys, name;
    if (blt.self.stop) {
    ...
    for (name in enemys) {
      ...
      if ((blt.x >= (enemy.x + enemy.width)) || ((blt.x + blt.width) <= enemy.x) || (blt.y >= (enemy.y + enemy.height)) || ((blt.y + blt.height) <= enemy.y)) {
        ...
        blt.self.alpha = 0;
        blt.self.stop = true;
        this.Hit(e, blt.self.fromid, blt.self.Direction);
        return true;
      }
    }
  };
  ...
  Player.prototype.Hit = function(e, fromid, Direction) {
  ...
  Player.prototype.gotHit = function(nums, Direction, name, time) {
...
Game = (function() {
  ...
  Game.prototype.note = function(type, x1, y1, size) {
    var name;
    if (type == null) {
      type = "HIT";
    }
...
Socket = (function() {
  function Socket(url1, myname, callback) {
    this.url = url1;
    this.myname = myname;
...
app.controller('HomeController', function($rootScope, $scope) {
  ...
  sock = new Socket(url, userid, function() {});
  ...
  sock.option.gotHit = function() {
    ...
    return playerlist[this.name].gotHit(this.hit, this.Direction, this.name, (new Date()).valueOf());
  };
