...
Vue.component("slider", {
  template: '<div> <div class="slider-list" v-bind:style="{transform: \'translate3d(\'+x+\'px,0,0)\',transitionDuration: duration+\'s\'}" > <slot/> </div> <div class="slider-left" @click="moveLeft"><img src="./img/arrow-left.png" /></div> <div class="slider-right" @click="moveRight"><img src="./img/arrow-left.png" /></div> </div>',
  data: function data() {
    return {
      root: null,
      moving: false,
  ...
  methods: {
    ...
    moveLeft: function moveLeft() {
      if (Math.abs(this.slideNumber) >= this.max || this.moved) {
        return false;
      }
      this.duration = 0.2;
      this.slideNumber = this.slideNumber - 1;
      this.x = -(Math.abs(this.slideNumber) * this.offset.w);
      return console.log(this.x, this.slideNumber);
    },
    moveRight: function moveRight() {
      if (Math.abs(this.slideNumber) <= 0 || this.moved) {
        return false;
      }
      this.duration = 0.2;
      this.slideNumber = this.slideNumber + 1;
      return this.x = -(Math.abs(this.slideNumber) * this.offset.w);
    },
