...
<register>
  <form onsubmit="{submit}" class="form">
    ...
    <div class="form-grounp">
      <label form="mobile">手机号码:</label>
      <input id="mobile" type="text" name="mobile"/>
    </div>
    <div class="form-grounp">
      <label form="email">电子邮箱:</label>
      <input id="email" type="text" name="email"/>
    </div>
    <div class="form-grounp">
      <label form="email">所在省/直辖市:</label>
      <div class="comb">
    ...
    <div class="form-grounp">
      <label form="buytime">购车时间:</label>
      <div class="select"><span>{buytimeName}</span>
        <select id="buytime" name="buytime" onchange="{changeSelect}">
          <option each="{name in buytimeData}" value="{name}">{name}</option>
        </select>
      </div>
    </div>
    <div class="form-grounp">
      <label form="buytime">购车预算:</label>
      <div class="select"><span>{howmuchName}</span>
        <select id="howmuch" name="howmuch" onchange="{changeSelect}">
          <option each="{name in howmuchData}" value="{name}">{name}</option>
        </select>
      </div>
    </div>
    <div class="form-btn">
      <button type="submit" class="submit"><img src="/Landrover/img/submit.jpg"/></button>
    </div>
  </form>
  <script>
    var self = this
    ...
    submit() {
    	...
    	$.post("http://api.giccoo.com/landrover/insert/",data,function(msg){
    		//- 提交
    		if (msg.recode == 200) {
    			alert("注册成功")
    		}else{
