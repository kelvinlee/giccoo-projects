...
var ANIMATION_END_NAME, ANIMATION_END_NAMES, BindShare, LoadFinished, TRANSITION_END_NAME, TRANSITION_END_NAMES, VENDORS, app, beginload, cdn, css3Prefix, defaultShare, fShare, handleFileLoad, hosts, i, loadList, mTestElement, my, preload, refreshShare, reloadWechat, shareFriend, shareTimeline, _Progress, _anim, _finishedlist, _i, _len, _wechat, _wechat_bool, _wechat_f,
...
app.controller('MainController', function($rootScope, $scope, $location, $http) {
  ...
  window.addEventListener((__indexOf.call(window, "onorientationchange") >= 0 ? "orientationchange" : "resize"), orientationChange, false);
  return window.cancelAnimationFrame(_anim);
});

app.controller('gameController', function($rootScope, $scope, $location, $timeout) {
  var Dripping, android, backtoNormal, checkHit, deviceMotionHandler, dishs, newdish, putTime, starTime, starUp, tis, _lastTime;
  ...
  checkHit = function() {
    var d, e, j, _j, _k, _ref, _ref1;
    if ($(".dropping").length > 0) {
      for (i = _j = 0, _ref = $(".dropping").length; 0 <= _ref ? _j < _ref : _j > _ref; i = 0 <= _ref ? ++_j : --_j) {
        d = $(".dropping").eq(i).offset();
        for (j = _k = 0, _ref1 = $("#dishs .item").length; 0 <= _ref1 ? _k < _ref1 : _k > _ref1; j = 0 <= _ref1 ? ++_k : --_k) {
