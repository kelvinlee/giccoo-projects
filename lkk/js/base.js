...
LoadFinished = function LoadFinished(name, $scope, callback) {
  var a, gotoNext, l, len1, len2, m;
  for (l = 0, len1 = _finishedlist.length; l < len1; l++) {
    a = _finishedlist[l];
    if (a.name === name) {
      a.load = true;
    }
  ...
  gotoNext = true;
  for (m = 0, len2 = _finishedlist.length; m < len2; m++) {
    a = _finishedlist[m];
    if (a.load === false) {
      gotoNext = false;
      break;
...
loadList = [{
  ...
  id: "dish-1",
  src: cdn + "lkk/img/dish-1.png"
}, {
  id: "dish-2",
  src: cdn + "lkk/img/dish-2.png"
}, {
  id: "dish-3",
  src: cdn + "lkk/img/dish-3.png"
}, {
  id: "dish-4",
...
refreshShare = function refreshShare(title, desc) {
  _wechat.title = "\u6211\u7528\u674E\u9526\u8BB0\u8D22\u795E\u869D\u6CB9\u5B8C\u6210\u4E86" + title + "\u76D8\u83DC\uFF0C\u6253\u8D25\u4E86" + desc + "%\u7684\u597D\u53CB\uFF01";
...
app.controller('MainController', function ($rootScope, $scope, $location, $http) {
  ...
  return window.cancelAnimationFrame(_anim);
});

app.controller('gameController', function ($rootScope, $scope, $location, $timeout) {
  var Dripping, _lastTime, android, backtoNormal, checkHit, deviceMotionHandler, dishs, newdish, putTime, starTime, starUp, tis;
  ...
  putTime = 0;
  tis = this;
  backtoNormal = function backtoNormal() {
    $(".wrap").css({
  ...
  Dripping = function Dripping(time) {
    var clone, e;
    // window.removeEventListener 'devicemotion',deviceMotionHandler
    if (time - putTime > 600) {
      // $(".log").html $("#Bottle").height()+","+$("#Bottle").offset().top+","
      putTime = time;
      if (preload.getResult("game-point") != null) {
      ...
      return clone.addClass("dropping");
    }
  };
  // backtoNormal()
  deviceMotionHandler = function deviceMotionHandler(eventData) {
    ...
    if (Math.abs(parseInt(acceleration.x * 100) - _lastTime.x) > 15) {
      ...
      if (android) {
        ...
        if (90 + acceleration.x * 6 > 100) {
          Dripping(new Date().getTime());
        }
      } else {
        $("#Bottle").css({
          "transform": "rotate(" + -(90 - acceleration.x * 6) + "deg)",
  ...
  this._checkDrop = function () {
    ...
    if (!starUp && $rootScope.CanRun) {
      return _anim = window.requestAnimationFrame(tis._checkDrop);
    }
  };
  this.addNewDish = function () {
    var e, item, lostTime;
    ...
    item[0].addEventListener(ANIMATION_END_NAME, function (e) {
      return $(this).remove();
    });
    $("#dishs").append(item);
    return tis.starFrom++;
  };
  checkHit = function checkHit() {
    var d, e, j, l, m, ref, ref1;
    if ($(".dropping").length > 0) {
      for (i = l = 0, ref = $(".dropping").length; 0 <= ref ? l < ref : l > ref; i = 0 <= ref ? ++l : --l) {
        d = $(".dropping").eq(i).offset();
        for (j = m = 0, ref1 = $("#dishs .item").length; 0 <= ref1 ? m < ref1 : m > ref1; j = 0 <= ref1 ? ++m : --m) {
          e = $("#dishs .item").eq(j).offset();
          if (d.left >= e.left + e.width || d.left + d.width <= e.left || d.top >= e.top + e.height || d.top + d.height <= e.top) {
            continue;
          } else {
            // $scope.$apply -> 
            $scope.score += 100;
  ...
  return window.cancelAnimationFrame(this._checkDrop);
});
...
app.controller('FormController', function ($rootScope, $scope, $location, $http) {
  $scope.formData = {};
