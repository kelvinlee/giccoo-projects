...
!function (a, b) {
  ...
  function e(b) {
    ...
    f.call(this, a), this.svgContainer = null, this.markers = {}, this.currentMarker = null, this.hoveringMarker = null, this.prop = { panelOpened: !1, panelOpening: !1, markersButton: this.psv.navbar.getNavbarButton("markers") }, this.create();
  }function h(a) {
    ...
    if (f.call(this, a), this.config = this.psv.config.navbar, this.items = [], this.config === !0) this.config = x.clone(e.DEFAULTS.navbar);else if ("string" == typeof this.config) this.config = this.config.split(" ");else if (!Array.isArray(this.config)) {
      console.warn('PhotoSphereViewer: hashmap form of "navbar" is deprecated, use an array instead.');var b = this.config;this.config = [];for (var c in b) {
        b[c] && this.config.push(c);
    ...
    n.call(this, a), this.zoom_range = null, this.zoom_value = null, this.prop = { mousedown: !1, buttondown: !1, longPressInterval: null }, this.create();
  }function v(a) {
    this.message = a, "captureStackTrace" in Error ? Error.captureStackTrace(this, v) : this.stack = new Error().stack;
  }function w(a, b) {
    if (!a.id) throw new v("missing marker id");if (a.image && (!a.width || !a.height)) throw new v("missing marker width/height");if ((a.image || a.html) && !(a.hasOwnProperty("x") && a.hasOwnProperty("y") || a.hasOwnProperty("latitude") && a.hasOwnProperty("longitude"))) throw new v("missing marker position, latitude/longitude or x/y");this.psv = b, this.visible = !0, this._dynamicSize = !1;var c,
        d = a.id,
        e = w.getType(a, !1);Object.defineProperties(this, { id: { configurable: !1, enumerable: !0, get: function get() {
          return d;
        }, set: function set(a) {} }, type: { configurable: !1, enumerable: !0, get: function get() {
          return e;
        }, set: function set(a) {} }, $el: { configurable: !1, enumerable: !0, get: function get() {
          return c;
        }, set: function set(a) {} }, _def: { configurable: !1, enumerable: !0, get: function get() {
          return this[e];
        }, set: function set(a) {
          this[e] = a;
    ...
    if (!this.config.usexmpdata) return b.resolved(null);var c = b(),
        d = new XMLHttpRequest(),
        e = this,
        f = 0;return d.onreadystatechange = function () {
      if (4 === d.readyState) {
        if (200 !== d.status && 201 !== d.status && 202 !== d.status && 0 !== d.status) throw e.container.textContent = "Cannot load image", new v("Cannot load image");e.loader.setProgress(100);var a = d.responseText,
            b = a.indexOf("<x:xmpmeta"),
            g = a.indexOf("</x:xmpmeta>"),
            h = a.substring(b, g);if (b === -1 || g === -1 || h.indexOf("GPano:") === -1) c.resolve(null);else {
          var i = { full_width: parseInt(x.getXMPValue(h, "FullPanoWidthPixels")), full_height: parseInt(x.getXMPValue(h, "FullPanoHeightPixels")), cropped_width: parseInt(x.getXMPValue(h, "CroppedAreaImageWidthPixels")), cropped_height: parseInt(x.getXMPValue(h, "CroppedAreaImageHeightPixels")), cropped_x: parseInt(x.getXMPValue(h, "CroppedAreaLeftPixels")), cropped_y: parseInt(x.getXMPValue(h, "CroppedAreaTopPixels")) };i.full_width && i.full_height && i.cropped_width && i.cropped_height ? c.resolve(i) : (console.warn("PhotoSphereViewer: invalid XMP data"), c.resolve(null));
        }
      } else 3 === d.readyState && e.loader.setProgress(f += 10);
    }, d.onprogress = function (a) {
      if (a.lengthComputable) {
        var b = parseInt(a.loaded / a.total * 100);b > f && (f = b, e.loader.setProgress(f));
      }
    }, d.onerror = function () {
    ...
    var b = [];if (Array.isArray(a)) {
      if (6 !== a.length) throw new v("Must provide exactly 6 image paths when using cubemap.");for (var c = 0; c < 6; c++) {
        b[c] = a[e.CUBE_MAP[c]];
      }a = b;
    } else if ("object" == (typeof a === "undefined" ? "undefined" : _typeof(a))) {
      if (!e.CUBE_HASHMAP.every(function (b) {
        return !!a[b];
      })) throw new v("Must provide exactly left, front, right, back, top, bottom when using cubemap.");e.CUBE_HASHMAP.forEach(function (c, d) {
        b[d] = a[c];
      }), a = b;
    }if (Array.isArray(a)) {
      if (this.prop.isCubemap === !1) throw new v("The viewer was initialized with an equirectangular panorama, cannot switch to cubemap.");return this.config.fisheye && console.warn("PhotoSphereViewer: fisheye effect with cubemap texture can generate distorsions."), this.config.cache_texture === e.DEFAULTS.cache_texture && (this.config.cache_texture *= 6), this.prop.isCubemap = !0, this._loadCubemapTexture(a);
    }if (this.prop.isCubemap === !0) throw new v("The viewer was initialized with an cubemap, cannot switch to equirectangular panorama.");return this.prop.isCubemap = !1, this._loadEquirectangularTexture(a);
  }, e.prototype._loadEquirectangularTexture = function (c) {
    if (this.config.cache_texture) {
      var d = this.getPanoramaCache(c);if (d) return this.prop.pano_data = d.pano_data, b.resolved(d.image);
    }return this._loadXMP(c).then(function (d) {
      var f = b(),
          g = new a.ImageLoader(),
          h = d ? 100 : 0;g.setCrossOrigin("anonymous");var i = function i(b) {
        h = 100, this.loader.setProgress(h), this.trigger("panorama-load-progress", c, h), !d && this.config.pano_data && (d = x.clone(this.config.pano_data)), d || (d = { full_width: b.width, full_height: b.height, cropped_width: b.width, cropped_height: b.height, cropped_x: 0, cropped_y: 0 }), this.prop.pano_data = d;var g,
            i = Math.min(d.full_width, e.SYSTEM.maxTextureWidth) / d.full_width;if (1 !== i || d.cropped_width != d.full_width || d.cropped_height != d.full_height) {
          var j = x.clone(d);j.full_width *= i, j.full_height *= i, j.cropped_width *= i, j.cropped_height *= i, j.cropped_x *= i, j.cropped_y *= i, b.width = j.cropped_width, b.height = j.cropped_height;var k = document.createElement("canvas");k.width = j.full_width, k.height = j.full_height;var l = k.getContext("2d");l.drawImage(b, j.cropped_x, j.cropped_y, j.cropped_width, j.cropped_height), g = new a.Texture(k);
        } else g = new a.Texture(b);g.needsUpdate = !0, g.minFilter = a.LinearFilter, g.generateMipmaps = !1, this.config.cache_texture && this._putPanoramaCache({ panorama: c, image: g, pano_data: d }), f.resolve(g);
      },
          j = function j(a) {
        if (a.lengthComputable) {
          var b = parseInt(a.loaded / a.total * 100);b > h && (h = b, this.loader.setProgress(h), this.trigger("panorama-load-progress", c, h));
        }
      },
          k = function k(a) {
    ...
    var d = b(),
        ...
        g = [0, 0, 0, 0, 0, 0],
        h = [],
        i = 0;f.setCrossOrigin("anonymous");for (var j = function j() {
      h.forEach(function (b) {
        b.needsUpdate = !0, b.minFilter = a.LinearFilter, b.generateMipmaps = !1;
      }), d.resolve(h);
    }, k = function k(b, d) {
      i++, g[b] = 100, this.loader.setProgress(x.sum(g) / 6), this.trigger("panorama-load-progress", c[b], g[b]);var f = Math.min(d.width, e.SYSTEM.maxTextureWidth / 2) / d.width;if (1 !== f) {
        var k = document.createElement("canvas");k.width = d.width * f, k.height = d.height * f;var l = k.getContext("2d");l.drawImage(d, 0, 0, k.width, k.height), h[b] = new a.Texture(k);
      } else h[b] = new a.Texture(d);this.config.cache_texture && this._putPanoramaCache({ panorama: c[b], image: h[b] }), 6 === i && j();
    }, l = function l(a, b) {
      if (b.lengthComputable) {
        var d = parseInt(b.loaded / b.total * 100);d > g[a] && (g[a] = d, this.loader.setProgress(x.sum(g) / 6), this.trigger("panorama-load-progress", c[a], g[a]));
      }
    }, m = function m(a, b) {
      throw this.container.textContent = "Cannot load image", d.reject(b), new v("Cannot load image " + a);
    }, n = 0; n < 6; n++) {
      if (this.config.cache_texture) {
        var o = this.getPanoramaCache(c[n]);if (o) {
          i++, g[n] = 100, h[n] = o.image;continue;
        }
      }f.load(c[n], k.bind(this, n), l.bind(this, n), m.bind(this, n));
    }return 6 === i && d.resolve(h), d.promise;
  }, e.prototype._setTexture = function (a) {
    if (this.scene || this._createScene(), this.prop.isCubemap) for (var b = 0; b < 6; b++) {
      this.mesh.material.materials[b].map && this.mesh.material.materials[b].map.dispose(), this.mesh.material.materials[b].map = a[b];
    } else this.mesh.material.map && this.mesh.material.map.dispose(), this.mesh.material.map = a;this.trigger("panorama-loaded"), this.render();
    ...
    var b = new a.SphereGeometry(e.SPHERE_RADIUS, e.SPHERE_VERTICES, e.SPHERE_VERTICES, -x.HalfPI),
        c = new a.MeshBasicMaterial({ side: a.DoubleSide, overdraw: e.SYSTEM.isWebGLSupported && this.config.webgl ? 0 : 1 });this.mesh = new a.Mesh(b, c), this.mesh.scale.x = -1, this.scene.add(this.mesh);
  }, e.prototype._createCubemap = function () {
    for (var b = new a.BoxGeometry(2 * e.SPHERE_RADIUS, 2 * e.SPHERE_RADIUS, 2 * e.SPHERE_RADIUS, e.CUBE_VERTICES, e.CUBE_VERTICES, e.CUBE_VERTICES), c = [], d = 0; d < 6; d++) {
      c.push(new a.MeshBasicMaterial({ overdraw: e.SYSTEM.isWebGLSupported && this.config.webgl ? 0 : 1 }));
    }this.mesh = new a.Mesh(b, new a.MultiMaterial(c)), this.mesh.position.x -= e.SPHERE_RADIUS, this.mesh.position.y -= e.SPHERE_RADIUS, this.mesh.position.z -= e.SPHERE_RADIUS, this.mesh.applyMatrix(new a.Matrix4().makeScale(1, 1, -1)), this.scene.add(this.mesh);var f = new a.MeshBasicMaterial({ side: a.BackSide, visible: !1 }),
        g = new a.Mesh(b, f);this.scene.add(g);
  }, e.prototype._transition = function (b, c) {
    if (this.prop.isCubemap) throw new v("Transition is not available with cubemap.");var d = this,
        f = new a.SphereGeometry(.9 * e.SPHERE_RADIUS, e.SPHERE_VERTICES, e.SPHERE_VERTICES, -x.HalfPI),
        g = new a.MeshBasicMaterial({ side: a.DoubleSide, overdraw: e.SYSTEM.isWebGLSupported && this.config.webgl ? 0 : 1, map: b, transparent: !0, opacity: 0 }),
        h = new a.Mesh(f, g);if (h.scale.x = -1, c) {
      h.rotateY(c.longitude - this.prop.longitude);var i = new a.Vector3(0, 1, 0).cross(this.camera.getWorldDirection()).normalize(),
          j = new a.Quaternion().setFromAxisAngle(i, c.latitude - this.prop.latitude);h.quaternion.multiplyQuaternions(j, h.quaternion);
    }return this.scene.add(h), this.render(), x.animation({ properties: { opacity: { start: 0, end: 1 } }, duration: d.config.transition.duration, easing: "outCubic", onTick: function onTick(a) {
    ...
    var a = this,
        ...
        c = this.config.longitude_range;this.config.longitude_range = null, x.animation({ properties: { speed: { start: this.config.anim_speed, end: 0 } }, duration: 300, easing: "inSine", onTick: function onTick(b) {
        a.config.anim_speed = b.speed;
      } }).then(function () {
      return x.animation({ properties: { speed: { start: 0, end: b } }, duration: 300, easing: "outSine", onTick: function onTick(b) {
          a.config.anim_speed = b.speed;
        } });
    }).then(function () {
    ...
    switch (a.type) {case "resize":
        x.throttle(this._onResize(), 50);break;case "keydown":
        this._onKeyDown(a);break;case "mousedown":
        this._onMouseDown(a);break;case "touchstart":
        this._onTouchStart(a);break;case "mouseup":
        this._onMouseUp(a);break;case "touchend":
        this._onTouchEnd(a);break;case "mousemove":
        this._onMouseMove(a);break;case "touchmove":
        this._onTouchMove(a);break;case e.SYSTEM.fullscreenEvent:
        this._fullscreenToggled();break;case e.SYSTEM.mouseWheelEvent:
    ...
    var b = 0,
        c = 0,
        d = 0,
        f = a.key || e.KEYMAP[a.keyCode || a.which];switch (f) {case "ArrowUp":
        c = .01;break;case "ArrowDown":
        c = -.01;break;case "ArrowRight":
        b = .01;break;case "ArrowLeft":
        b = -.01;break;case "PageUp":case "+":
        d = 1;break;case "PageDown":case "-":
        d = -1;}0 !== d ? this.zoom(this.prop.zoom_lvl + d) : 0 === c && 0 === b || this.rotate({ longitude: this.prop.longitude + b * this.prop.move_speed * this.prop.hFov, latitude: this.prop.latitude + c * this.prop.move_speed * this.prop.vFov });
    ...
    var b = [{ x: parseInt(a.touches[0].clientX), y: parseInt(a.touches[0].clientY) }, { x: parseInt(a.touches[1].clientX), y: parseInt(a.touches[1].clientY) }];this.prop.pinch_dist = Math.sqrt(Math.pow(b[0].x - b[1].x, 2) + Math.pow(b[0].y - b[1].y, 2)), this.prop.moving = !1, this.prop.zooming = !0;
    ...
    var b = this,
        c = { x: a.clientX - this.prop.mouse_history[0][1], y: a.clientY - this.prop.mouse_history[0][2] },
        d = Math.sqrt(c.x * c.x + c.y * c.y);this.prop.animation_promise = x.animation({ properties: { clientX: { start: a.clientX, end: a.clientX + c.x }, clientY: { start: a.clientY, end: a.clientY + c.y } }, duration: d * e.INERTIA_WINDOW / 100, easing: "outCirc", onTick: function onTick(a) {
        b._move(a, !1);
      } }).ensure(function () {
      b.prop.moving = !1;
    });
    ...
    var b = this.container.getBoundingClientRect(),
        c = { target: a.target, client_x: a.clientX, client_y: a.clientY, viewer_x: parseInt(a.clientX - b.left), viewer_y: parseInt(a.clientY - b.top) },
        d = this.viewerCoordsToVector3({ x: c.viewer_x, y: c.viewer_y });if (d) {
      var f = this.vector3ToSphericalCoords(d);if (c.longitude = f.longitude, c.latitude = f.latitude, !this.prop.isCubemap) {
        var g = this.sphericalCoordsToTextureCoords({ longitude: c.longitude, latitude: c.latitude });c.texture_x = g.x, c.texture_y = g.y;
    ...
    if (this.prop.moving) {
      var c = parseInt(a.clientX),
          d = parseInt(a.clientY);this.rotate({ longitude: this.prop.longitude - (c - this.prop.mouse_x) / this.prop.size.width * this.prop.move_speed * this.prop.hFov, latitude: this.prop.latitude + (d - this.prop.mouse_y) / this.prop.size.height * this.prop.move_speed * this.prop.vFov }), this.prop.mouse_x = c, this.prop.mouse_y = d, b !== !1 && this._logMouseMove(a);
    }
  }, e.prototype._zoom = function (a) {
    if (this.prop.zooming) {
      var b = [{ x: parseInt(a.touches[0].clientX), y: parseInt(a.touches[0].clientY) }, { x: parseInt(a.touches[1].clientX), y: parseInt(a.touches[1].clientY) }],
          c = Math.sqrt(Math.pow(b[0].x - b[1].x, 2) + Math.pow(b[0].y - b[1].y, 2)),
          d = 80 * (c - this.prop.pinch_dist) / this.prop.size.width;this.zoom(this.prop.zoom_lvl + d), this.prop.pinch_dist = c;
    }
  }, e.prototype._onMouseWheel = function (a) {
    a.preventDefault(), a.stopPropagation();var b = void 0 !== a.deltaY ? -a.deltaY : void 0 !== a.wheelDelta ? a.wheelDelta : -a.detail;if (0 !== b) {
      var c = parseInt(b / Math.abs(b));this.zoom(this.prop.zoom_lvl + c);
    }
  }, e.prototype._fullscreenToggled = function () {
    var a = this.isFullscreenEnabled();this.config.keyboard && (a ? this.startKeyboardControl() : this.stopKeyboardControl()), this.trigger("fullscreen-updated", a);
  }, e.prototype._logMouseMove = function (a) {
    var b = Date.now();this.prop.mouse_history.push([b, a.clientX, a.clientY]);for (var c = null, d = 0; d < this.prop.mouse_history.length;) {
      this.prop.mouse_history[0][d] < b - e.INERTIA_WINDOW ? this.prop.mouse_history.splice(d, 1) : c && this.prop.mouse_history[0][d] - c > e.INERTIA_WINDOW / 10 ? (this.prop.mouse_history.splice(0, d), d = 0, c = this.prop.mouse_history[0][d]) : (d++, c = this.prop.mouse_history[0][d]);
    }
  }, e.prototype.load = function () {
    if (!this.config.panorama) throw new v("No value given for panorama.");return this.setPanorama(this.config.panorama, !1);
    ...
    this._stopAll();var a = this,
        b = null,
        c = null;!function d(e) {
      e && (c = null === b ? 0 : e - b, b = e, a.rotate({ longitude: a.prop.longitude + a.config.anim_speed * c / 1e3, latitude: a.prop.latitude - (a.prop.latitude - a.config.anim_lat) / 200 })), a.prop.autorotate_reqid = window.requestAnimationFrame(d);
    ...
    this._stopAll();var a = this;!function b() {
      a.doControls.update(), a.prop.direction = a.camera.getWorldDirection();var c = a.vector3ToSphericalCoords(a.prop.direction);a.prop.longitude = c.longitude, a.prop.latitude = c.latitude, a.render(!1), a.prop.orientation_reqid = window.requestAnimationFrame(b);
    ...
    if (this._stopAll(), !c) return this.rotate(a), b.resolved();if (this.cleanPosition(a), this.applyRanges(a).forEach(this.trigger.bind(this, "_side-reached")), !c && "number" != typeof c) {
      c = c ? x.parseSpeed(c) : this.config.anim_speed;var d = Math.acos(Math.cos(this.prop.latitude) * Math.cos(a.latitude) * Math.cos(this.prop.longitude - a.longitude) + Math.sin(this.prop.latitude) * Math.sin(a.latitude));c = d / c * 1e3;
    }var e = x.getShortestArc(this.prop.longitude, a.longitude);return this.prop.animation_promise = x.animation({ properties: { longitude: { start: this.prop.longitude, end: this.prop.longitude + e }, latitude: { start: this.prop.latitude, end: a.latitude } }, duration: c, easing: "inOutSine", onTick: this.rotate.bind(this) }), this.prop.animation_promise;
    ...
    if (!this.config.cache_texture) throw new v("Cannot preload panorama, cache_texture is disabled");return this._loadTexture(a);
  }, e.prototype.clearPanoramaCache = function (a) {
    if (!this.config.cache_texture) throw new v("Cannot clear cache, cache_texture is disabled");if (a) {
      for (var b = 0, c = this.prop.cache.length; b < c; b++) {
        if (this.prop.cache[b].panorama === a) {
          this.prop.cache.splice(b, 1);break;
        }
      }
    } else this.prop.cache.length = 0;
  }, e.prototype.getPanoramaCache = function (a) {
    if (!this.config.cache_texture) throw new v("Cannot query cache, cache_texture is disabled");return this.prop.cache.filter(function (b) {
      return b.panorama === a;
    }).shift();
  }, e._loadSystem = function () {
    var a = e.SYSTEM;a.loaded = !0, a.pixelRatio = window.devicePixelRatio || 1, a.isWebGLSupported = x.isWebGLSupported(), a.isCanvasSupported = x.isCanvasSupported(), a.maxTextureWidth = a.isWebGLSupported ? x.getMaxTextureWidth() : 4096, a.mouseWheelEvent = x.mouseWheelEvent(), a.fullscreenEvent = x.fullscreenEvent(), a.deviceOrientationSupported = b(), "DeviceOrientationEvent" in window ? window.addEventListener("deviceorientation", e._deviceOrientationListener, !1) : a.deviceOrientationSupported.reject();
    ...
    if (this.prop.isCubemap) throw new v("Unable to use texture coords with cubemap.");var b = (a.x + this.prop.pano_data.cropped_x) / this.prop.pano_data.full_width * x.TwoPI,
        c = (a.y + this.prop.pano_data.cropped_y) / this.prop.pano_data.full_height * Math.PI;return { longitude: b >= Math.PI ? b - Math.PI : b + Math.PI, latitude: x.HalfPI - c };
  }, e.prototype.sphericalCoordsToTextureCoords = function (a) {
    if (this.prop.isCubemap) throw new v("Unable to use texture coords with cubemap.");var b = a.longitude / x.TwoPI * this.prop.pano_data.full_width,
        c = a.latitude / Math.PI * this.prop.pano_data.full_height;return { x: parseInt(a.longitude < Math.PI ? b + this.prop.pano_data.full_width / 2 : b - this.prop.pano_data.full_width / 2) - this.prop.pano_data.cropped_x, y: parseInt(this.prop.pano_data.full_height / 2 - c) - this.prop.pano_data.cropped_y };
    ...
    var b = Math.acos(a.y / Math.sqrt(a.x * a.x + a.y * a.y + a.z * a.z)),
        c = Math.atan2(a.x, a.z);return { longitude: c < 0 ? -c : x.TwoPI - c, latitude: x.HalfPI - b };
  }, e.prototype.viewerCoordsToVector3 = function (b) {
    var c = new a.Vector2(2 * b.x / this.prop.size.width - 1, -2 * b.y / this.prop.size.height + 1);this.raycaster.setFromCamera(c, this.camera);var d = this.raycaster.intersectObjects(this.scene.children);return 1 === d.length ? d[0].point : null;
  }, e.prototype.vector3ToViewerCoords = function (a) {
    return a = a.clone(), a.project(this.camera), { x: parseInt((a.x + 1) / 2 * this.prop.size.width), y: parseInt((1 - a.y) / 2 * this.prop.size.height) };
    ...
    switch (a.type) {case "mouseenter":
        this._onMouseEnter(a);break;case "mouseleave":
        this._onMouseLeave(a);break;case "mousemove":
        this._onMouseMove(a);break;case "click":
        this._onClick(a.args[0], a, !1);break;case "dblclick":
        this._onClick(a.args[0], a, !0);break;case "render":
        this.renderMarkers();break;case "open-panel":
        this._onPanelOpened();break;case "close-panel":
        this._onPanelClosed();}
  }, g.prototype.addMarker = function (a, b) {
    if (!a.id) throw new v("missing marker id");if (this.markers[a.id]) throw new v('marker "' + a.id + '" already exists');var c = new w(a, this.psv);return c.isNormal() ? this.container.appendChild(c.$el) : this.svgContainer.appendChild(c.$el), this.markers[c.id] = c, b !== !1 && this.renderMarkers(), c;
  }, g.prototype.getMarker = function (a) {
    var b = "object" == (typeof a === "undefined" ? "undefined" : _typeof(a)) ? a.id : a;if (!this.markers[b]) throw new v('cannot find marker "' + b + '"');return this.markers[b];
  }, g.prototype.getCurrentMarker = function () {
    return this.currentMarker;
  }, g.prototype.updateMarker = function (a, b) {
    var c = this.getMarker(a);return c.update(a), b !== !1 && this.renderMarkers(), c;
    ...
    var a = [];for (var b in this.markers) {
      a.push(this.markers[b]);
    }var c = this.psv.config.templates.markersList({ markers: this.psv.change("render-markers-list", a), config: this.psv.config });this.prop.panelOpening = !0, this.psv.panel.showPanel(c, !0), this.psv.panel.container.querySelector(".psv-markers-list").addEventListener("click", this._onClickItem.bind(this));
  }, g.prototype.hideMarkersList = function () {
    this.prop.panelOpened && this.psv.panel.hidePanel();
  }, g.prototype.renderMarkers = function () {
    var b = this.psv.isGyroscopeEnabled() ? a.Math.radToDeg(this.psv.camera.rotation.z) : 0;for (var c in this.markers) {
      var d = this.markers[c],
          e = d.visible;if (e && d.isPolygon()) {
        var f = this._getPolygonPositions(d);if (e = f.length > 2) {
          d.position2D = this._getPolygonDimensions(d, f);var g = "";f.forEach(function (a) {
            g += a.x + "," + a.y + " ";
          }), d.$el.setAttributeNS(null, "points", g);
        }
      } else if (e) {
        var h = this._getMarkerPosition(d);e = this._isMarkerVisible(d, h), e && (d.position2D = h, d.$el instanceof SVGElement ? d.$el.setAttribute("transform", "translate(" + h.x + " " + h.y + ")" + (!d.lockRotation && b ? " rotate(" + b + " " + d.anchor.left * d.width + " " + d.anchor.top * d.height + ")" : "")) : (d.$el.style.transform = "translate3D(" + h.x + "px, " + h.y + "px, 0px)" + (!d.lockRotation && b ? " rotateZ(" + b + "deg)" : ""), d.$el.style.transformOrigin = 100 * d.anchor.left + "% " + 100 * d.anchor.top + "%"));
    ...
    if (a._dynamicSize) {
      x.toggleClass(a.$el, "psv-marker--transparent", !0);var b = a.$el.getBoundingClientRect();x.toggleClass(a.$el, "psv-marker--transparent", !1), a.width = b.right - b.left, a.height = b.bottom - b.top;
    }var c = this.psv.vector3ToViewerCoords(a.position3D);return c.x -= a.width * a.anchor.left, c.y -= a.height * a.anchor.top, c;
    ...
    var b = a.positions3D.length,
      ...
      return { vector: a, visible: a.dot(this.psv.prop.direction) > 0 };
        ...
        d = [];return c.forEach(function (a, e) {
      if (!a.visible) {
        var f = [0 === e ? c[b - 1] : c[e - 1], e === b - 1 ? c[0] : c[e + 1]];f.forEach(function (b) {
          b.visible && d.push({ visible: b, invisible: a, index: e });
        });
      }
    }), d.reverse().forEach(function (a) {
      c.splice(a.index, 0, { vector: this._getPolygonIntermediaryPoint(a.visible.vector, a.invisible.vector), visible: !0 });
    ...
    var d = this.psv.prop.direction.clone().normalize(),
        ...
        i = new a.Vector3().crossVectors(h, d);return h.applyAxisAngle(i, .01).multiplyScalar(e.SPHERE_RADIUS);
  }, g.prototype._getPolygonDimensions = function (a, b) {
    var c = +(1 / 0),
        d = +(1 / 0),
        e = -(1 / 0),
        f = -(1 / 0);return b.forEach(function (a) {
    ...
    var b;a.target && (b = a.target.psvMarker) && !b.isPolygon() && (this.hoveringMarker = b, this.psv.trigger("over-marker", b), b.tooltip && this.psv.tooltip.showTooltip({ content: b.tooltip.content, position: b.tooltip.position, left: b.position2D.x, top: b.position2D.y, box: { width: b.width, height: b.height } }));
  }, g.prototype._onMouseLeave = function (a) {
    var b;if (a.target && (b = a.target.psvMarker)) {
      if (b.isPolygon() && a.relatedTarget && x.hasParent(a.relatedTarget, this.psv.tooltip.container)) return;this.psv.trigger("leave-marker", b), this.hoveringMarker = null, this.psv.tooltip.hideTooltip();
    }
  }, g.prototype._onMouseMove = function (a) {
    if (!this.psv.prop.moving) {
      var b;if (a.target && (b = a.target.psvMarker) && b.isPolygon() || a.target && x.hasParent(a.target, this.psv.tooltip.container) && (b = this.hoveringMarker)) {
        this.hoveringMarker || (this.psv.trigger("over-marker", b), this.hoveringMarker = b);var c = this.psv.container.getBoundingClientRect();b.tooltip && this.psv.tooltip.showTooltip({ content: b.tooltip.content, position: b.tooltip.position, top: a.clientY - c.top - this.psv.config.tooltip.arrow_size / 2, left: a.clientX - c.left - this.psv.config.tooltip.arrow_size, box: { width: 2 * this.psv.config.tooltip.arrow_size, height: 2 * this.psv.config.tooltip.arrow_size } });
    ...
    f.prototype.create.call(this), this.loader = document.createElement("div"), this.loader.className = "psv-loader", this.container.appendChild(this.loader), this.canvas = document.createElement("canvas"), this.canvas.className = "psv-loader-canvas", this.canvas.width = this.loader.clientWidth, this.canvas.height = this.loader.clientWidth, this.loader.appendChild(this.canvas), this.tickness = (this.loader.offsetWidth - this.loader.clientWidth) / 2;var a;if (this.psv.config.loading_img ? (a = document.createElement("img"), a.className = "psv-loader-image", a.src = this.psv.config.loading_img) : this.psv.config.loading_txt && (a = document.createElement("div"), a.className = "psv-loader-text", a.innerHTML = this.psv.config.loading_txt), a) {
      var b = Math.round(Math.sqrt(2 * Math.pow(this.canvas.width / 2 - this.tickness / 2, 2)));a.style.maxWidth = b + "px", a.style.maxHeight = b + "px", this.loader.appendChild(a);
    }
  }, h.prototype.destroy = function () {
    delete this.loader, delete this.canvas, f.prototype.destroy.call(this);
  }, h.prototype.setProgress = function (a) {
    var b = this.canvas.getContext("2d");b.clearRect(0, 0, this.canvas.width, this.canvas.height), b.lineWidth = this.tickness, b.strokeStyle = x.getStyle(this.loader, "color"), b.beginPath(), b.arc(this.canvas.width / 2, this.canvas.height / 2, this.canvas.width / 2 - this.tickness / 2, -Math.PI / 2, a / 100 * 2 * Math.PI - Math.PI / 2), b.stroke();
    ...
    f.prototype.create.call(this), this.config.forEach(function (a) {
      if ("object" == (typeof a === "undefined" ? "undefined" : _typeof(a))) this.items.push(new p(this, a));else switch (a) {case o.id:
          this.items.push(new o(this));break;case u.id:
          this.items.push(new u(this));break;case q.id:
          this.items.push(new q(this));break;case t.id:
          this.items.push(new t(this));break;case r.id:
          this.items.push(new r(this));break;case s.id:
          this.psv.config.gyroscope && this.items.push(new s(this));break;case "caption":
          this.items.push(new j(this, this.psv.config.caption));break;case "spacer":
          a = "spacer-5";default:
          var b = a.match(/^spacer\-([0-9]+)$/);if (null === b) throw new v("Unknown button " + a);this.items.push(new k(this, b[1]));}
    }, this);
    ...
    var b = null;return this.items.some(function (c) {
    ...
    f.prototype.create.call(this), this.container.innerHTML = '<div class="psv-panel-resizer"></div><div class="psv-panel-close-button"></div><div class="psv-panel-content"></div>', this.content = this.container.querySelector(".psv-panel-content");var a = this.container.querySelector(".psv-panel-close-button");a.addEventListener("click", this.hidePanel.bind(this)), this.psv.config.mousewheel && this.container.addEventListener(e.SYSTEM.mouseWheelEvent, function (a) {
      a.stopPropagation();
    });var b = this.container.querySelector(".psv-panel-resizer");b.addEventListener("mousedown", this), b.addEventListener("touchstart", this), this.psv.container.addEventListener("mouseup", this), this.psv.container.addEventListener("touchend", this), this.psv.container.addEventListener("mousemove", this), this.psv.container.addEventListener("touchmove", this);
    ...
    switch (a.type) {case "mousedown":
        this._onMouseDown(a);break;case "touchstart":
        this._onTouchStart(a);break;case "mousemove":
        this._onMouseMove(a);break;case "touchmove":
        this._onTouchMove(a);break;case "mouseup":
        this._onMouseUp(a);break;case "touchend":
        this._onMouseUp(a);}
    ...
    var b = parseInt(a.clientX),
        c = parseInt(a.clientY);this.container.style.width = this.container.offsetWidth - (b - this.prop.mouse_x) + "px", this.prop.mouse_x = b, this.prop.mouse_y = c;
  }, m.prototype = Object.create(f.prototype), m.prototype.constructor = m, m.className = "psv-tooltip", m.publicMethods = ["showTooltip", "hideTooltip", "isTooltipVisible"], m.leftMap = { 0: "left", .5: "center", 1: "right" }, m.topMap = { 0: "top", .5: "center", 1: "bottom" }, m.prototype.create = function () {
    ...
    this.prop.timeout && (window.clearTimeout(this.prop.timeout), this.prop.timeout = null);var b = this.isTooltipVisible(),
        c = this.container,
        d = this.content,
        e = this.arrow;if (a.position || (a.position = ["top", "center"]), a.box || (a.box = { width: 0, height: 0 }), "string" == typeof a.position) {
      var f = x.parsePosition(a.position);if (!(f.left in m.leftMap && f.top in m.topMap)) throw new v('unable to parse tooltip position "' + tooltip.position + '"');a.position = [m.topMap[f.top], m.leftMap[f.left]];
    }if ("center" == a.position[0] && "center" == a.position[1]) throw new v('unable to parse tooltip position "center center"');if (b) for (var g = c.classList.length - 1; g >= 0; g--) {
      var h = c.classList.item(g);"psv-tooltip" != h && "psv-tooltip--visible" != h && c.classList.remove(h);
    } else c.className = "psv-tooltip";a.className && x.addClasses(c, a.className), d.innerHTML = a.content, c.style.top = "0px", c.style.left = "0px";var i = c.getBoundingClientRect(),
        j = { posClass: a.position.slice(), width: i.right - i.left, height: i.bottom - i.top, top: 0, left: 0, arrow_top: 0, arrow_left: 0 };this._computeTooltipPosition(j, a);var k = !1;if (j.top < this.config.offset ? (j.posClass[0] = "bottom", k = !0) : j.top + j.height > this.psv.prop.size.height - this.config.offset && (j.posClass[0] = "top", k = !0), j.left < this.config.offset ? (j.posClass[1] = "right", k = !0) : j.left + j.width > this.psv.prop.size.width - this.config.offset && (j.posClass[1] = "left", k = !0), k && this._computeTooltipPosition(j, a), c.style.top = j.top + "px", c.style.left = j.left + "px", e.style.top = j.arrow_top + "px", e.style.left = j.arrow_left + "px", c.classList.add("psv-tooltip--" + j.posClass.join("-")), !b) {
      var l = this;this.prop.timeout = window.setTimeout(function () {
    ...
    if (this.prop.timeout && (window.clearTimeout(this.prop.timeout), this.prop.timeout = null), this.isTooltipVisible()) {
      this.container.classList.remove("psv-tooltip--visible");var a = this;this.prop.timeout = window.setTimeout(function () {
    ...
    var c = !1;switch (a.posClass[0]) {case "bottom":
        a.top = b.top + b.box.height + this.config.offset + this.config.arrow_size, a.arrow_top = 2 * -this.config.arrow_size, c = !0;break;case "center":
        a.top = b.top + b.box.height / 2 - a.height / 2, a.arrow_top = a.height / 2 - this.config.arrow_size;break;case "top":
        a.top = b.top - a.height - this.config.offset - this.config.arrow_size, a.arrow_top = a.height, c = !0;}switch (a.posClass[1]) {case "right":
        c ? (a.left = b.left + b.box.width / 2 - this.config.offset - this.config.arrow_size, a.arrow_left = this.config.offset) : (a.left = b.left + b.box.width + this.config.offset + this.config.arrow_size, a.arrow_left = 2 * -this.config.arrow_size);break;case "center":
        a.left = b.left + b.box.width / 2 - a.width / 2, a.arrow_left = a.width / 2 - this.config.arrow_size;break;case "left":
    ...
    var a = document.createElement("a");a.href = this.psv.config.panorama, a.download = this.psv.config.panorama, this.psv.container.appendChild(a), a.click();
    ...
    switch (a.type) {case "fullscreen-updated":
        this.toggleActive(a.args[0]);}
    ...
    switch (a.type) {case "gyroscope-updated":
        this.toggleActive(a.args[0]);}
  }, s.prototype._onClick = function () {
    this.psv.toggleGyroscopeControl();
  }, s.prototype._onAvailabilityChange = function (a) {
    if (a) {
      if (!x.checkTHREE("DeviceOrientationControls")) throw new v("Missing Three.js components: DeviceOrientationControls. Get them from three.js-examples package.");this.show();
    }
    ...
    switch (a.type) {case "mousedown":
        this._initZoomChangeWithMouse(a);break;case "touchstart":
        this._initZoomChangeByTouch(a);break;case "mousemove":
        this._changeZoomWithMouse(a);break;case "touchmove":
        this._changeZoomByTouch(a);break;case "mouseup":
        this._stopZoomChange(a);break;case "touchend":
        this._stopZoomChange(a);break;case "zoom-updated":
    ...
    if (this.prop.mousedown) {
      var b = parseInt(a) - this.zoom_range.getBoundingClientRect().left,
          c = b / this.zoom_range.offsetWidth * 100;this.psv.zoom(c);
    }
    ...
    var c = [];if (w.types.forEach(function (b) {
      a[b] && c.push(b);
    }), 0 === c.length && !b) throw new v("missing marker content, either " + w.types.join(", "));if (c.length > 1) throw new v("multiple marker content, either " + w.types.join(", "));return c[0];
    ...
    if (a && a !== this) {
      var b = w.getType(a, !0);if (void 0 !== b && b !== this.type) throw new v("cannot change marker type");x.deepmerge(this, a);
    ...
    switch (this._dynamicSize = !0, this.type) {case "rect":
        "number" == typeof this._def ? this._def = { x: 0, y: 0, width: this._def, height: this._def } : Array.isArray(this._def) ? this._def = { x: 0, y: 0, width: this._def[0], height: this._def[1] } : this._def.x = this._def.y = 0;break;case "circle":
        "number" == typeof this._def ? this._def = { cx: this._def, cy: this._def, r: this._def } : Array.isArray(this._def) ? this._def = { cx: this._def[0], cy: this._def[0], r: this._def[0] } : this._def.cx = this._def.cy = this._def.r;break;case "ellipse":
        "number" == typeof this._def ? this._def = { cx: this._def, cy: this._def, rx: this._def, ry: this._def } : Array.isArray(this._def) ? this._def = { cx: this._def[0], cy: this._def[1], rx: this._def[0], ry: this._def[1] } : (this._def.cx = this._def.rx, this._def.cy = this._def.ry);break;case "path":
        "string" == typeof this._def && (this._def = { d: this._def });}Object.getOwnPropertyNames(this._def).forEach(function (a) {
    ...
    this._dynamicSize = !0, this.svgStyle ? Object.getOwnPropertyNames(this.svgStyle).forEach(function (a) {
      ...
      if (a && "object" != _typeof(a[0])) for (var b = 0; b < a.length; b++) {
        a.splice(b, 2, [a[b], a[b + 1]]);
      }
    }), this.polygon_px ? this.polygon_rad = this.polygon_px.map(function (a) {
      var b = this.psv.textureCoordsToSphericalCoords({ x: a[0], y: a[1] });return [b.longitude, b.latitude];
  ...
  };var x = {};return e.Utils = x, x.TwoPI = 2 * Math.PI, x.HalfPI = Math.PI / 2, x.svgNS = "http://www.w3.org/2000/svg", x.checkTHREE = function (b) {
    ...
    var a = document.createElement("canvas"),
        ...
        c = null;return a.getContext && b.some(function (b) {
      try {
    ...
    var a = x.getWebGLCtx();if (null !== a) return a.getParameter(a.MAX_TEXTURE_SIZE);
    ...
    if (a.classList) void 0 === c ? a.classList.toggle(b) : c && !a.classList.contains(b) ? a.classList.add(b) : c || a.classList.remove(b);else {
      var d = a.getAttribute("class") || "",
          e = d.indexOf(b) !== -1,
          f = new RegExp("(?:^|\\s)" + b + "(?:\\s|$)");void 0 !== c && !c || e ? c || (d = d.replace(f, " ")) : d += d.length > 0 ? " " + b : b, a.setAttribute("class", d);
    }
    ...
    do {
      if (a === b) return !0;
    } while (a = a.parentNode);return !1;
  }, x.getClosest = function (a, b) {
    var c = a.matches || a.msMatchesSelector;do {
      if (c.bind(a)(b)) return a;
    } while (a = a.parentElement);return null;
  }, x.mouseWheelEvent = function () {
    return "onwheel" in document.createElement("div") ? "wheel" : void 0 !== document.onmousewheel ? "mousewheel" : "DOMMouseScroll";
  }, x.fullscreenEvent = function () {
    var a = { exitFullscreen: "fullscreenchange", webkitExitFullscreen: "webkitfullscreenchange", mozCancelFullScreen: "mozfullscreenchange", msExitFullscreen: "msFullscreenEnabled" };for (var b in a) {
    ...
    var c;return null !== (c = a.match("<GPano:" + b + ">(.*)</GPano:" + b + ">")) ? c[1] : null !== (c = a.match("GPano:" + b + '="(.*?)"')) ? c[1] : null;
    ...
    var c = [0, x.TwoPI, -x.TwoPI];return c.reduce(function (c, d) {
      return d = b - a + d, Math.abs(d) < Math.abs(c) ? d : c;
    }, 1 / 0);
  }, x.parsePosition = function (a) {
    if (!a) return { top: .5, left: .5 };if ("object" == (typeof a === "undefined" ? "undefined" : _typeof(a))) return a;var b = a.toLocaleLowerCase().split(" ").slice(0, 2);1 === b.length && (b = void 0 !== x.parsePosition.positions[b[0]] ? [b[0], "center"] : [b[0], b[0]]);var c = "left" != b[1] && "right" != b[1] && "top" != b[0] && "bottom" != b[0];b = b.map(function (a) {
      return x.parsePosition.positions[a] || a;
    }), c || b.reverse();var d = b.join(" ").match(/^([0-9.]+)% ([0-9.]+)%$/);return d ? { left: d[1] / 100, top: d[2] / 100 } : { top: .5, left: .5 };
  }, x.parsePosition.positions = { top: "0%", bottom: "100%", left: "0%", right: "100%", center: "50%" }, x.parseSpeed = function (b) {
    if ("string" == typeof b) {
      b = b.toString().trim();var c = parseFloat(b.replace(/^(-?[0-9]+(?:\.[0-9]*)?).*$/, "$1")),
          d = b.replace(/^-?[0-9]+(?:\.[0-9]*)?(.*)$/, "$1").trim();switch (d.match(/(pm|per minute)$/) && (c /= 60), d) {case "dpm":case "degrees per minute":case "dps":case "degrees per second":
          b = a.Math.degToRad(c);break;case "radians per minute":case "radians per second":
          b = c;break;case "rpm":case "revolutions per minute":case "rps":case "revolutions per second":
          b = c * x.TwoPI;break;default:
          throw new v('unknown speed unit "' + d + '"');}
    }return b;
  }, x.parseAngle = function (b, c) {
    if ("string" == typeof b) {
      var d = b.toLowerCase().trim().match(/^(-?[0-9]+(?:\.[0-9]*)?)(.*)$/);if (!d) throw new v('unknown angle "' + b + '"');var e = parseFloat(d[1]),
          f = d[2];if (f) switch (f) {case "deg":case "degs":
          b = a.Math.degToRad(e);break;case "rad":case "rads":
          b = e;break;default:
    ...
    function c(b) {
      if (d.promise.getStatus() !== -1) {
        null === e && (e = b);var f,
            g = (b - e) / a.duration,
            h = {};if (g < 1) {
          for (f in a.properties) {
            h[f] = a.properties[f].start + (a.properties[f].end - a.properties[f].start) * a.easing(g);
          }a.onTick(h, g), window.requestAnimationFrame(c);
        } else {
          for (f in a.properties) {
            h[f] = a.properties[f].end;
        ...
        e = null;a.easing && "string" != typeof a.easing || (a.easing = x.animation.easings[a.easing || "linear"]), void 0 !== a.delay ? window.setTimeout(function () {
      window.requestAnimationFrame(c);
    }, a.delay) : window.requestAnimationFrame(c);var f = d.promise;return f.cancel = function () {
      d.reject();
    }, f;
  }, x.animation.easings = { linear: function linear(a) {
      return a;
    ...
    var c,
      ...
      var i = Date.now();g || (g = i);var j = b - (i - g);return c = this, d = arguments, j <= 0 || j > b ? (f && (clearTimeout(f), f = null), g = i, e = a.apply(c, d), f || (c = d = null)) : f || (f = setTimeout(h, j)), e;
    };
  }, x.isPlainObject = function (a) {
    if ("object" == (typeof a === "undefined" ? "undefined" : _typeof(a)) && null !== a) {
      if ("function" == typeof Object.getPrototypeOf) {
        var b = Object.getPrototypeOf(a);return b === Object.prototype || null === b;
      }return "[object Object]" == Object.prototype.toString.call(a);
    }return !1;
  }, x.deepmerge = function (a, b) {
    var c = b;return function d(a, b) {
      return Array.isArray(b) ? (a && Array.isArray(a) ? a.length = 0 : a = [], b.forEach(function (b, c) {
        a[c] = d(null, b);
      })) : "object" == (typeof b === "undefined" ? "undefined" : _typeof(b)) ? (a && !Array.isArray(a) || (a = {}), Object.keys(b).forEach(function (e) {
        "object" == _typeof(b[e]) && b[e] && x.isPlainObject(b[e]) ? b[e] != c && (a[e] ? d(a[e], b[e]) : a[e] = d(null, b[e])) : a[e] = b[e];
      })) : a = b, a;
    }(a, b);
  }, x.clone = function (a) {
    ...
    if (a.requestAnimationFrame = a.requestAnimationFrame || a.mozRequestAnimationFrame || a.webkitRequestAnimationFrame || a.msRequestAnimationFrame, a.cancelAnimationFrame = a.cancelAnimationFrame || a.mozCancelAnimationFrame || a.webkitCancelAnimationFrame || a.msCancelAnimationFrame, !a.requestAnimationFrame) {
      var b,
          c = [],
          d = [],
          e = 0;a.requestAnimationFrame = function (a) {
        return c.push([++e, a]), b || (b = setInterval(function () {
          if (c.length) {
            var a = +new Date(),
                e = d;for (d = c, c = e; d.length;) {
              d.shift()[1](a);
            }
          } else clearInterval(b), b = void 0;
        }, 20)), e;
      }, a.cancelAnimationFrame = function (a) {
        var b, e;for (b = 0, e = c.length; b < e; b += 1) {
