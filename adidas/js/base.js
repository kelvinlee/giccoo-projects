...
app.run(function ($rootScope, $location) {
  ...
  return $rootScope.$on('$routeChangeSuccess', function () {
    ...
    if (newPage === "men1" && oldPage === "") {
      // return $rootScope.from = "same" if oldPage is "-1"
      // return $rootScope.from = "same" if newPage is oldPage
      return $rootScope.from = "left";
    }
    if (newPage === "women1" && oldPage === "") {
      return $rootScope.from = "right";
    }
...
app.controller('MainController', function ($rootScope, $scope, $location, $http) {
  var orientationChange, s1, s2;
  // console.log "aa"
  $rootScope.CanRun = true;
  if ($("body").height() <= 440) {
    $("body").addClass("iphone4");
  }
  if ($("body").height() > 500 || $("body").width() > 320) {
    s1 = $("body").height() / 500;
...
app.controller('swipeController', function ($rootScope, $scope, $location) {
  ...
  $scope.runPageX = function (bool) {
    ...
    return $scope.$apply(function () {
      if (bool) {
        return $location.path("/men1");
      } else {
        return $location.path("/women1");
      }
    });
  };
...
app.controller('menController', function ($scope, $location, $timeout) {
  var tis;
  this.note = false;
  tis = this;
  this.popshow = false;
...
app.controller('womenController', function ($scope, $location, $timeout) {
  var tis;
  this.note = false;
  tis = this;
  this.popshow = false;
...
app.directive("parallay", function ($location) {
  return {
    restrict: 'EA',
