...
var ANIMATION_END_NAME, ANIMATION_END_NAMES, Loader, SendNote, Store, TRANSITION_END_NAME, TRANSITION_END_NAMES, UpdateShare, UpdateShareContent, VENDORS, _loader, cdn, closeWaiting, css3Prefix, debug, defaultWords, global, hideShareWechat, i, isMM, j, layzr, len, link, loadStart, mTestElement, namelist, opendWaiting, qrcode, selectFiles, sending, share, shareCall, shareEnd, shareOneToAll, shared, tm, uploadImage;
...
// 	"谁说旅行就是拍别人?"
// ]
defaultWords = ["谁说素颜不能当女神？", "谁说90后=非主流？", "谁说旅行晒照是种病？", "谁说美食不该手机先吃？"];
...
window.onload = function () {
    ...
    $("#title").on("keyup", function () {
        if ($("#title").val().length > 0) {
            return $("#title").next().hide();
        } else {
            return $("#title").next().show();
        }
    });
    ...
    return $.get('http://api.giccoo.com/sayno/momo/config', {
...
uploadImage = function uploadImage(next) {
    var createFullImage, data, _postFunc;
    if (sending) {
    ...
    return _postFunc = function postFunc() {
        return $.post(link + "/sayno/build/create", data, function (msg) {
            sending = false;
            if (msg.recode === 200) {
                // alert "Success"
                // showShare()
...
shareEnd = function shareEnd(msg) {
    if (global.homepage != null) {
        // alert "share back:"+msg.status
        // if msg.status is 0
        return global.homepage.openShare();
    }
};
