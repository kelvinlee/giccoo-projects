...
var ANIMATION_END_NAME, ANIMATION_END_NAMES, Store, TRANSITION_END_NAME, TRANSITION_END_NAMES, VENDORS, _history, begin, controllers, css3Prefix, getStyle, hideShareWechat, i, j, layzr, len, loadStart, mTestElement, qrcode, routeFun, routeParser, tm, wonShare;
...
getStyle = function(obj, className) {
  if (obj.currentStyle) {
    return obj.currentStyle(className);
  } else {
    return document.defaultView.getComputedStyle(obj, false)[className];
  }
};
...
window.onload = function() {
  var ep;
  if (getStyle(document.getElementById("qrcode"), "display") === "block") {
    qrcode();
    return false;
  }
  ep = $(".load-progress .n");
