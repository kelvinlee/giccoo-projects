...
var ANIMATION_END_NAME, ANIMATION_END_NAMES, GetStep, Loader, SendNote, Store, TRANSITION_END_NAME, TRANSITION_END_NAMES, UpdateCircle, UpdateLucky, VENDORS, _loader, _startDate, _step, _stepList, cdn, css3Prefix, debug, global, i, isMM, j, layzr, len, link, lotterySuccess, mTestElement, mainSlider, pc, qrcode, runInApp, secondSlider, tabId, tabId2, taskData, tm, uid, updateMedal, updateStep;
...
GetStep = function GetStep() {
    return ldl.callNativeTmp("getDailyStatsWithData", JSON.stringify({
        ...
        var _taskOverDate, dailystats, res, steps;
        res = JSON.parse(d);
...
runInApp = function runInApp(msg) {
...
window.onload = function () {
    ...
    $(".homepage .banner").on("click", function () {
    ...
    $(".pop.faild .content").on("click", function () {
    ...
    // Set content to text
    return $(".pop.success .content").on("click", function () {
...
UpdateCircle = function UpdateCircle(h) {
    var Fstart, Sstart;
    if (h > 100) {
        h = 100;
    }
    ...
    return $(".circleProgress_wrapper .leftcircle").css({
        "-webkit-transform": "rotate(" + Sstart + "deg)",
        "transform": "rotate(" + Sstart + "deg)"
    });
};

UpdateLucky = function UpdateLucky(temp) {
    var Fstart;
    // 890 水壶
    // 930 谢谢参与
    // 960 小米手环
    ...
    if (temp === 930) {
        return setTimeout(function () {
            return $(".pop.faild").removeClass("hide");
        }, 4700);
    } else {
        return setTimeout(function () {
            return $(".pop.success-form").removeClass("hide");
        }, 4700);
    }
};

lotterySuccess = function lotterySuccess() {
    console.log("success");
    $(".pop.success-form").addClass("hide");
    return $(".pop.success").removeClass("hide");
};

updateMedal = function updateMedal() {
    ...
    if (first) {
        ...
        return $(".infopage .next-step .normal").addClass("hide");
    }
};
...
updateStep = function updateStep(step) {
    var all, medal1, medal2, medal3;
    all = Math.ceil(step / 120000 * 100);
    UpdateCircle(all);
    $("#stepnumber").text(step);
    medal1 = localStorage.getItem("medal-1") ? false : true;
    medal2 = localStorage.getItem("medal-2") ? false : true;
    medal3 = localStorage.getItem("medal-3") ? false : true;
    if (step < 40000) {
        $("#nextstepnumber").text(_stepList[0]);
    } else if (step > 40000 && step < 70000) {
        $("#nextstepnumber").text(_stepList[1]);
        updateMedal(1, medal1);
        localStorage.setItem("medal-1", "true");
        _step = 1;
    } else if (step > 70000 && step < 120000) {
